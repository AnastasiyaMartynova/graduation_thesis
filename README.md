...
Дипломная работа по автоматизированному анализу коллекции математических статей в формате LaTeX. Реализованы методы тематического моделирования (LSI, LDA, BERTopic, BigARTM) и автоматического реферирования на основе LLM (Ollama + Qwen2.5). Для демонстрации результатов разработан графический интерфейс на основе Tkinter.
...
├── graphic_interface.py        # Графический интерфейс для демонстрации результатов
│
...
6. Интерактивная демонстрация всех результатов через графический интерфейс
...
Модуль `graphic_interface.py` реализует десктопное приложение на Tkinter для интерактивного просмотра и сравнения результатов всех этапов работы.

### Вкладки интерфейса

- **Обзор** - общая информация о проекте и статус доступных выходных файлов
- **Корпус** - статистика предобработанного корпуса (число документов, токенов, частотное распределение)
- **Темы** - просмотр тем каждой модели (LDA, LSI, BERTopic, BigARTM) с ключевыми словами и весами
- **Сравнение** - сравнение метрик качества моделей; детальные таблицы результатов grid search и подбора гиперпараметров
- **Реферирование** - просмотр сгенерированных аннотаций по каждому документу с метриками ROUGE и BERTScore

### Запуск

Интерфейс читает данные из папок `outputs/` и `models/`, поэтому перед запуском необходимо выполнить все ноутбуки (01–06).

```bash
# В основном окружении (venv)
python graphic_interface.py
```

> Для корректного отображения тем BigARTM потребуется предварительно запустить `05_bigartm.ipynb` в окружении `venv_bigartm`.
...
### Основное окружение (ноутбуки 01–04, 06 и графический интерфейс)
...
06_summarization       →  outputs/summaries.csv
       ...
       ↓
graphic_interface.py   ←  читает все выходные файлы из outputs/ и models/
```
...
Графический интерфейс использует только стандартную библиотеку Python (`tkinter`, `threading`) и не требует дополнительной установки.
